...
function parse_csv_to_apartments($csv_data) {
    ...
    for ($i = 4; $i < count($lines); $i++) {
        ...
        // Маппинг колонок согласно вашей структуре в Google Sheets
        $apartment = [
            'этаж' => (int)$data[0],                    // Колонка A - этаж
            'номер' => (int)$data[1],                   // Колонка B - номер
            'площадь' => (float)$data[4],               // Колонка E - общая площадь
            'вид' => trim($data[5]),                    // Колонка F - вид
            'цена_м2' => (int)$data[6],                 // Колонка G - цена за м²
            'общая_сумма' => (int)$data[7],             // Колонка H - общая сумма
            'статус' => 'Свободный'                     // По умолчанию все свободные
        ];
