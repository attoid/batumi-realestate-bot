...
function parse_csv_to_apartments($csv_data) {
    ...
    for ($i = 1; $i < count($lines); $i++) {
        ...
        // Парсим CSV строку (учитываем возможные запятые в данных)
        $data = str_getcsv($line);
        
        // Проверяем что у нас достаточно колонок
        if (count($data) < 7) continue;
        
        // Маппинг колонок (адаптируйте под вашу структуру в Google Sheets)
        $apartment = [
            'этаж' => (int)$data[0],           // Колонка A - этаж
            'номер' => (int)$data[1],          // Колонка B - номер
            'площадь' => (float)$data[2],      // Колонка C - площадь
            'вид' => trim($data[3]),           // Колонка D - вид
            'цена_м2' => (int)$data[4],        // Колонка E - цена за м²
            'общая_сумма' => (int)$data[5],    // Колонка F - общая сумма
            'статус' => trim($data[6])         // Колонка G - статус
        ];
        
        // Добавляем только если данные корректные
        if ($apartment['номер'] > 0 && $apartment['площадь'] > 0) {
...
function get_fallback_apartments() {
    return [
        ...
        // Добавьте несколько ключевых квартир как резерв
    ];
}
